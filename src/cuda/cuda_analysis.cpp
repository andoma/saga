  ...
  size_t alloc(size_t reqsize)
  {
    ...
    if(it == m_sizes.end()) {
      ...
      if(size > reqsize) {
        ...
        m_sizes.insert(std::make_pair(size - reqsize, offset + reqsize));
        m_chunks.insert(std::make_pair(offset + reqsize, size - reqsize));
      }
    }
    return offset;
  }
  ...
  void update() {
    ...
    while(!stable) {
      ...
      for(ssize_t i = m_ops.size() - 1; i >= 0; i--) {
        ...
        if(memcmp(cur->m_out, out_prim, m_ln_words * sizeof(uint32_t)) ||
    ...
    while(1) {
      ...
      for(ssize_t i = m_ops.size() - 1; i >= 0; i--) {
        ...
        if(kill) {
          ...
          for(int j = 0; j < m_ln_words; j++) {
            if((m_lns[i]->m_out[j] & m_lns[i]->m_def[j]) != m_lns[i]->m_def[j]) {
