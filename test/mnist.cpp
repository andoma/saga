...
makeLabeledImages(const uint8_t *images, const uint8_t *labels,
                  size_t image_step, size_t count)
{
...
mnist_main(int argc, char **argv)
{
    ...
    auto train_data = makeLabeledImages(
        &train_image_data[16], &train_label_data[8], cols * rows, train_images);

    auto test_data = makeLabeledImages(
        &test_image_data[16], &test_label_data[8], cols * rows, test_images);
    ...
    auto input = makeTensor(Tensor::DataType::U8,
                            Dims({DimParam::BATCH_SIZE, 1, rows, cols}));
